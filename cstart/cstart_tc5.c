...
**      and the application C variables for core 5.                       *
**                                                                        *
...
#if ! defined CPU5_CORE_ID

#pragma message "cstart_tc5.c: This derivative does not support core 5"

#else // ! defined CPU5_CORE_ID
...
#include "cstart_tc5.h"				/* include configuration */

#ifndef __NO_VTC

#if __USE_ARGC_ARGV
#pragma noclear					/* bss clearing not before cinit */
static char argcv_tc5[__ARGCV_BUFSIZE] __align(4);
#pragma clear
extern int _argcv( const char *, size_t );
#endif

/* linker definitions */
extern __far void _lc_ue_ustack_tc5[];	/* user stack end */
extern __far void _lc_ue_istack_tc5[];	/* interrupt stack end */
extern __far void _lc_u_int_tab_tc5[];	/* interrupt table */
extern __far void _lc_u_trap_tab_tc5[];	/* trap table */
extern __far void _SMALL_DATA_TC5[];       /* centre of A0 addressable area */
extern __far void _LITERAL_DATA_TC5[];     /* centre of A1 addressable area */
extern __far void _A8_DATA_TC5[];          /* centre of A8 addressable area */
extern __far void _A9_DATA_TC5[];          /* centre of A9 addressable area */

/* external functions */
#if __C_INIT
extern void _c_init_tc5(void);		/* C initialization function */
#endif
#ifdef __CALL_INIT
extern void _call_init_tc5(void);		/* call a user function before main() */
#endif
#ifdef _CALL_ENDINIT
extern void _endinit_tc5(void);		/* call a user function with protection switched off */
#endif
...
extern void _endinit_clear_tc5(void) { endinit_set(_ENDINIT_DISABLE); }
extern void _endinit_set_tc5  (void) { endinit_set(_ENDINIT_ENABLE); }
...
extern char _lc_ub_ustack_tc5[];
#define STACK_LIMIT	(_lc_ub_ustack_tc5 + 64)
#endif

void __noreturn__ __jump__ _start_tc5( void )
{
        __init_sp();
}
...
#if  __USP_INIT
	void * sp = (void *)((unsigned int)(_lc_ue_ustack_tc5) & STACK_ALIGN);
...
static void __noinline__ __noreturn__ __jump__ _start( void )
{
	...
	/*
	 * Clear the ENDINIT bit in the SCU_WDTCPU5CON0 register in order
	 ...
	 */
#if __BTV_INIT
        __mtcr(BTV,  (unsigned int)_lc_u_trap_tab_tc5);
#endif
	 ...
	 * Single Entry Vector Table, for core tc1.6.x or tc1.6.2, is supported
         * by _sevt_isr_tc5() calling interrupt handlers installed
	 * with _sevt_isr_install() stored in _sevt_isrs_tc5 list.
	 * The _sevt_isr_tc5() is located at interrupt vector table
	 * entry 64. The start address of the interrupt vector table
	 * is defined by linker definition INTTAB5.
	 */
#pragma extern _sevt_isr_tc5
	__mtcr(BIV, (unsigned int)(_lc_u_int_tab_tc5) | (0xff<<3) | 1 );
#else
	...
	__mtcr(BIV, (unsigned int)(_lc_u_int_tab_tc5) | 1 );	/* 8 Byte vector spacing */
#else
	__mtcr(BIV, (unsigned int)(_lc_u_int_tab_tc5));		/* 32 Byte vector spacing */
#endif
#endif
#endif
	 ...
	 */
#if __ISP_INIT
	unsigned int isp = (unsigned int)(_lc_ue_istack_tc5) & STACK_ALIGN;
	__mtcr(ISP, isp);
#endif
	...
	/*
	 * Set the ENDINIT bit in the SCU_WDTCPU5CON0 register again
	...
	endinit_set(_ENDINIT_ENABLE);
         ...
         */
#if __A0A1_INIT
        void * a0 = _SMALL_DATA_TC5;
        __asm( "mov.aa\ta0,%0"::"a"(a0) );

        void * a1 = _LITERAL_DATA_TC5;
         ...
         */
#if __A8A9_INIT
        void * a8 = _A8_DATA_TC5;
        __asm( "mov.aa\ta8,%0"::"a"(a8) );

        void * a9 = _A9_DATA_TC5;
...
# error "Unexpected value for macro __CSAS_FOR_FCD_TRAP, allowed values are: 1-4"
	...
	extern int _lc_ub_csa_tc5[][16];    /* context save area 1 begin */
	extern int _lc_ue_csa_tc5[][16];    /* context save area 1 end   */
	static __far int (* const csa_area_begin[])[16] = { _lc_ub_csa_tc5};
	static __far int (* const csa_area_end[])[16]   = { _lc_ue_csa_tc5};
	...
	if ( &_tcx_end_c_init )
	{
		__swapmskw( (unsigned int *)&_tcx_end_c_init, 1<<5, 1<<5 );	/* tc5 end of initialization */
		/*
	 ...
	 */
#if __USE_ARGC_ARGV
	exit( main( _argcv( argcv_tc5, __ARGCV_BUFSIZE), (char **)argcv_tc5) );
#else
 ...
 *
 * DESCRIPTION:  Sets or Clears the ENDINIT bit in the SCU_WDTCPU5CON0
...
static void inline endinit_set( _endinit_t endinit_value )
{
	...
	/*
	 * 1st step: Password access (create password and send to SCU_WDTCPU5CON0)
	 */        
	wdt_con0 = SCU_WDTCPU5CON0.U;
	...
	SCU_WDTCPU5CON0.U = wdt_con0;

	/*
	 * 2nd step: Modify access, set the bit ENDINIT to 1 or 0 to allow access to
	 *           registers: SCU_WDTCPU5CON1, BTV, BIV, ISP and mod_CLC
	 *
	 */
	...
	__isync();
	SCU_WDTCPU5CON0.U = wdt_con0;          
	SCU_WDTCPU5CON0.U;         /* read is required */
}
...
void __noreturn__ __jump__ __start_tc5_no_vtc( void )
{
        __init_sp();
}
...
# error "Unexpected value for macro __CSAS_FOR_FCD_TRAP, allowed values are: 1-4"
	...
	{
		__swapmskw( (unsigned int *)&_lc_t_tc0__tcx_end_c_init, 1<<5, 1<<5 );	/* tc5 end of initialization */
		/*
		 * Before calling main, which has code core association share,
		 * wait until all cores have done the global initializations
		 * to avoid race conditions on initialized shared global data.
