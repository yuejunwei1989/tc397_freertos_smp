...
#if ! defined CPU1_CORE_ID

#pragma message "cstart_tc1.c: This derivative does not support core 1"

#else // ! defined CPU1_CORE_ID
...
static char argcv_tc1[__ARGCV_BUFSIZE] __align(4);
#pragma clear
...
extern char _lc_ub_ustack_tc1[];
#define STACK_LIMIT	(_lc_ub_ustack_tc1 + 64)
#endif
...
#if  __USP_INIT
	void * sp = (void *)((unsigned int)(_lc_ue_ustack_tc1) & STACK_ALIGN);
...
# error "Unexpected value for macro __CSAS_FOR_FCD_TRAP, allowed values are: 1-4"
	 ...
	 */
#if __USE_ARGC_ARGV
	exit( main( _argcv( argcv_tc1, __ARGCV_BUFSIZE), (char **)argcv_tc1) );
#else
...
void __noreturn__ __jump__ __start_tc1_no_vtc( void )
{
        __init_sp();
}
...
# error "Unexpected value for macro __CSAS_FOR_FCD_TRAP, allowed values are: 1-4"
	...
	{
		__swapmskw( (unsigned int *)&_lc_t_tc0__tcx_end_c_init, 1<<1, 1<<1 );	/* tc1 end of initialization */
		/*
		 * Before calling main, which has code core association share,
		 * wait until all cores have done the global initializations
		 * to avoid race conditions on initialized shared global data.
