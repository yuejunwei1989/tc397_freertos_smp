...
#if ! defined CPU2_CORE_ID

#pragma message "cstart_tc2.c: This derivative does not support core 2"

#else // ! defined CPU2_CORE_ID
...
static char argcv_tc2[__ARGCV_BUFSIZE] __align(4);
#pragma clear
...
extern char _lc_ub_ustack_tc2[];
#define STACK_LIMIT	(_lc_ub_ustack_tc2 + 64)
#endif
...
# error "Unexpected value for macro __CSAS_FOR_FCD_TRAP, allowed values are: 1-4"
	 ...
	 */
#if __USE_ARGC_ARGV
	exit( main( _argcv( argcv_tc2, __ARGCV_BUFSIZE), (char **)argcv_tc2) );
#else
...
void __noreturn__ __jump__ __start_tc2_no_vtc( void )
{
        __init_sp();
}
...
# error "Unexpected value for macro __CSAS_FOR_FCD_TRAP, allowed values are: 1-4"
	...
	{
		__swapmskw( (unsigned int *)&_lc_t_tc0__tcx_end_c_init, 1<<2, 1<<2 );	/* tc2 end of initialization */
		/*
		 * Before calling main, which has code core association share,
		 * wait until all cores have done the global initializations
		 * to avoid race conditions on initialized shared global data.
